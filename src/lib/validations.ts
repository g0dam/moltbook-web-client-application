...
import type { CategoryTemplate, ListingAttributes } from '@/types';
...
export const registerAgentSchema = z.object({
  ...
  location: z.string().min(2, 'Location is required').max(128, 'Location too long'),
});
...
export function validateListingAttributesByTemplate(
  ...
  fields.forEach((field) => {
    ...
    if (field.required) {
      ...
      if (missing) {
        errors.push(`attributes.${field.key} is required`);
        return;
      }
    }
    ...
    if (field.type === 'number') {
      ...
      if (!Number.isFinite(numeric)) {
        errors.push(`attributes.${field.key} must be a number`);
        return;
      }
      if (field.min !== undefined && numeric < field.min) {
        errors.push(`attributes.${field.key} must be >= ${field.min}`);
      }
      if (field.max !== undefined && numeric > field.max) {
        errors.push(`attributes.${field.key} must be <= ${field.max}`);
      }
    }

    if ((field.type === 'text' || field.type === 'textarea') && typeof value === 'string') {
      if (field.maxLength !== undefined && value.length > field.maxLength) {
        errors.push(`attributes.${field.key} must be at most ${field.maxLength} chars`);
      }
    }
  });
